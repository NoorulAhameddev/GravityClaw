...
async function showBackupStats() {
  try {
    ...
    console.log(`  Storage used: ${(stats.totalSize / 1024 / 1024 / 1024).toFixed(2)} GB`);
  ...
  try {
    ...
    if (err instanceof Error) {
      if (err.message.includes('ENOSPC')) {
        ...
        console.error('   Action: Check backup directory permissions');
      } else if (err.message.includes('database is locked')) {
        console.error('❌ Backup failed: Database is locked');
        console.error('   Action: Stop other processes accessing database');
      } else {
  ...
  setInterval(
    async () => {
      ...
      if (backups.length > 0) {
        ...
        if (!result.valid) {
          console.error(`⚠️  Latest backup is corrupted: ${result.message}`);
...
function logBackupStatus() {
  ...
  if (status.lastBackupTime) {
    const hoursSinceBackup =
      (Date.now() - new Date(status.lastBackupTime).getTime()) / (1000 * 60 * 60);

    if (hoursSinceBackup > 25) {
      console.warn(`⚠️  No backup in ${hoursSinceBackup.toFixed(1)} hours`);
    }
  }
