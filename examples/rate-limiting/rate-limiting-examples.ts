...
📊 Rate Limit Status:
  ...
  Percentage used: ${Math.round(
    ((status.limit.requestsPerMinute - status.tokensAvailable) / status.limit.requestsPerMinute) *
      100,
  )}%
 ...
 */
export async function example3_toolExecution(sessionId: string, toolName: string) {
  // 1. Check rate limit
...
🎙️ Voice Tool Usage:
  ...
  Success rate: ${(((voiceHistory.length - voiceViolations) / voiceHistory.length) * 100).toFixed(
    2,
  )}%
  `);
...
export async function example7_adaptiveRetry(
  ...
  while (attempt < maxRetries) {
    ...
    if (attempt < maxRetries) {
      ...
      const backoffSeconds = Math.min(status.retryAfter, Math.pow(2, attempt));
      log.info(`Rate limited, retrying in ${backoffSeconds}s (attempt ${attempt}/${maxRetries})`);
...
export function example8_dashboardData() {
  ...
  const violations = history.filter((h) => !h.allowed).length;
  const avgRequestsPerMinute = history.length / Math.max(1, Math.floor(history.length / 100));
...
function getToolCategory(toolName: string): string {
  const categories: Record<string, string[]> = {
    voice: ['text_to_speech', 'speak', 'set_voice', 'enable_talk_mode', 'disable_talk_mode'],
