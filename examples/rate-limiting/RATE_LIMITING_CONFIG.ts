...
export const RATE_LIMIT_DEFAULTS = {
  session: {
    ...
    description: 'Overall limit per user session. Includes all tools and operations.',
  },
  ...
  voice: {
    ...
    description: 'Text-to-speech, voice synthesis, and audio generation. Limited due to API costs.',
    tools: 'text_to_speech, speak, set_voice, enable_talk_mode, disable_talk_mode',
  },
  ...
  memory: {
    ...
    tools:
      'save_fact, recall_facts, save_entity, save_relationship, query_graph, search_memory_semantic',
  },
...
export const ENV_CONFIGURATION = {
  NODE_ENV: {
    description: "Environment setting. Set to 'development' for 10x higher limits.",
    values: {
      development: 'Development mode - 10x higher limits for testing (100->1000, 50->500, etc.)',
      production: 'Production mode - standard limits apply',
    },
...
export const DATABASE_CONFIGURATION = {
  tables: {
    rate_limits: {
      description: 'Stores current rate limit bucket state for recovery after restarts',
      columns: [
        'id - Primary key',
    ...
    rate_limit_history: {
      ...
      note: 'This table can grow large. Implement cleanup jobs to purge old entries beyond 30 days.',
    },
  },
  ...
  cleanup: {
    ...
    historyRecords: {
      ...
      command: 'DELETE FROM rate_limit_history WHERE created_at < (current_timestamp - 3600000)',
    },
  },
};
...
export const CUSTOMIZATION_GUIDE = {
  ...
  approach2_devMode: {
    description: 'Use NODE_ENV=development for testing (10x higher limits)',
    steps: [
    ...
    pros: ['No code changes, no restart needed (set before start)', 'Easy to test'],
    cons: ['Only for development, not for production'],
    example: 'NODE_ENV=development npm run dev',
  },

  approach3_userCustom: {
    description: 'Let users set their own lower limits via update_rate_limits tool',
    steps: [
    ...
    pros: ['User control, no admin intervention needed', 'Stored in database'],
    cons: ['Users can only lower limits, not increase', 'Requires tool execution'],
...
export const SCALING = {
  ...
  withRedis: {
    description: 'Distributed rate limiting (recommended for production)',
...
export const EXAMPLE_CONFIGS = {
  ...
  testing: {
    description: 'Testing setup - only in-memory, fast cleanup, 5x higher limits',
    modifications: {
      DEFAULT_CONFIGS: {
  ...
  production: {
    description: 'Production setup - normal limits, persistence enabled, error logging',
    NODE_ENV: 'production',
    LOG_LEVEL: 'warn',
  ...
  restrictive: {
    description: 'Restrictive setup - lower limits for testing quota enforcement',
    modifications: {
      DEFAULT_CONFIGS: {
