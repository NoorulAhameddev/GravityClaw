...
export function Sidebar({ currentPage, onNavigate, status }: SidebarProps) {
    return (
        <div className="w-[240px] bg-surface backdrop-blur-xl border-r border-border/50 flex flex-col flex-shrink-0 h-screen overflow-y-auto relative">
            <div className="absolute inset-0 bg-gradient-to-b from-accent/5 to-transparent pointer-events-none" />
            
            <div className="p-5 pt-[22px] pb-[16px] text-lg font-bold border-b border-border/50 tracking-tight relative">
            ...
            <nav className="flex-1 py-3 px-2">
                {NAV_ITEMS.map((item, i) => {
                    if (item.type === 'section') {
                        return (
                            <div key={i} className="px-3 pt-4 pb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-dark">
                                {item.label}
                            </div>
                        );
                    }
                    ...
                    return (
                        ...
                        >
                            <Icon size={16} className={cn("flex-shrink-0", active && "drop-shadow-[0_0_6px_rgba(255,255,255,0.5)]")} />
            ...
            <div className="mt-auto p-4 border-t border-border/50 flex items-center gap-3 text-xs text-muted backdrop-blur-sm bg-bg/30">
                <div className={cn(
