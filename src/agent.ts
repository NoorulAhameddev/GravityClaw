...
export async function runAgent(options: AgentRunOptions): Promise<AgentRunResult> {
  ...
  const executor =
    options.dependencies.toolExecutor ?? new ToolExecutor(options.dependencies.toolRegistry);
    ...
    return await orchestrator.run(
      {
        ...
        maxTotalToolCalls:
          options.maxTotalToolCalls ?? options.dependencies.config.AGENT_MAX_TOOLS_TOTAL,
...
export function isMeaningfulProgress(result: string): boolean {
  ...
  if (
    ...
    normalized.includes('no errors') ||
    normalized.includes('not failed')
  )
    return true;
