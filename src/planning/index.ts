...
export function formatPlanForPrompt(plan: ExecutionPlan): string {
  ...
  for (let i = 0; i < plan.steps.length; i++) {
    ...
    const statusIcon =
      step.status === 'completed' ? '[x]' : step.status === 'active' ? '[>]' : '[ ]';
...
- Respond ONLY with valid JSON, no additional text`;
  ...
  try {
      ...
      [
        {
          role: 'system',
          content:
            'You are a JSON-generating planning system. Always respond with valid JSON only.',
        },
        ...history,
      ],
      [],
    ...
    const plan: ExecutionPlan = {
      ...
      steps,
      finalResponseStyle:
        (parsed.finalResponseStyle as ExecutionPlan['finalResponseStyle']) || 'conversational',
      currentStepIndex: 0,
      createdAt: now,
      updatedAt: now,
    };
