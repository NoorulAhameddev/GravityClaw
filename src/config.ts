...
const envSchema = z.object({
  ...
  LLM_FAILOVER_LIST: z
    ...
    .describe(
      "Comma-separated list of providers to use in failover mode (e.g., 'openai,anthropic,groq')",
    ),

  // API Keys for different providers (at least one required based on LLM_PROVIDER)
  OPENCODEZEN_API_KEY: z.string().optional(),
  OPENCODEZEN_BASE_URL: z.string().optional().default('https://opencode.ai/zen/v1'),
  ...
  ELEVENLABS_VOICE_ID: z
    ...
    .default('bella')
    .describe(
      'Default ElevenLabs voice ID (bella, eric, essie, isabella, james, jessica, josh, kunka, lah, michael, rachel, samantha, sarah, william)',
    ),
  ...
  WAKE_WORD_PHRASE: z
    ...
    .default('hey claw')
    .describe(
      "Custom wake phrase (default: 'hey claw'). Use built-in words like 'go', 'stop', 'yes', 'no' for better accuracy.",
    ),
  ...
  MASTER_KEY: z
    ...
    .optional()
    .describe(
      'Master encryption key for secrets.enc.json (64-char hex or any string) - generate with: npm run secret:generate',
    ),
  ...
  SAFE_DIRECTORIES: z
    ...
    .default('.')
    .describe(
      'Comma-separated list of allowed directories for file operations (default: workspace only)',
    ),
  ...
  HEALTH_API_KEY: z
    ...
    .optional()
    .describe(
      'Separate API key for health check endpoints (allows Docker HEALTHCHECK, k8s probes)',
    ),
  ...
  SLACK_APP_ID: z.string().optional().describe('Slack app ID for identifying the app (optional)'),
  ...
  NOTION_API_KEY: z
    ...
    .optional()
    .describe(
      'Notion API key for workspace integration (get from https://www.notion.so/my-integrations)',
    ),
  ...
  EMAIL_SMTP_USER: z.string().optional().describe('SMTP username (usually your email address)'),
  EMAIL_SMTP_PASS: z.string().optional().describe('SMTP password or app-specific password'),
  EMAIL_IMAP_HOST: z
    .string()
    .optional()
  ...
  EMAIL_IMAP_USER: z.string().optional().describe('IMAP username (usually your email address)'),
  EMAIL_IMAP_PASS: z.string().optional().describe('IMAP password or app-specific password'),
  EMAIL_FROM_ADDRESS: z.string().optional().describe('Email address to send from (your email)'),
  EMAIL_ALLOWED_SENDERS: z
    .string()
    .optional()
    .describe(
      'Comma-separated list of allowed email addresses (only these senders will be processed)',
    ),
  ...
  PG_HOST: z.string().optional().default('localhost').describe('PostgreSQL host'),
  ...
  PG_DATABASE: z.string().optional().default('gravityclaw').describe('PostgreSQL database name'),

  PG_USER: z.string().optional().default('postgres').describe('PostgreSQL user'),

  PG_PASSWORD: z.string().optional().describe('PostgreSQL password'),
  ...
  LOG_FORMAT: z.string().optional().default('text').describe('Log format: text | json'),
  ...
  PATH_ALLOWLIST: z.string().optional().describe('Comma-separated list of allowed directories'),
  ...
  SERPAPI_API_KEY: z.string().optional().describe('SerpAPI key for search'),

  BRAVE_SEARCH_KEY: z.string().optional().describe('Brave Search API key'),
  ...
  REDIS_URL: z.string().optional().describe('Redis URL for queue backend'),
  ...
  WEBHOOK_BASE_URL: z.string().optional().describe('Base URL for webhooks'),

  // Backup System Configuration
  BACKUP_DIR: z.string().optional().default('backups').describe('Directory for backup files'),
  BACKUP_CRON: z
    .string()
    .optional()
    .default('0 2 * * *')
...
if (process.env.NODE_ENV === 'production' && config.UNRESTRICTED_ACCESS) {
  ...
  console.error(
    '  • This flag bypasses path validation and allows unauthenticated reads/writes to any system file.',
  );
  console.error(
    '\n→ Disable UNRESTRICTED_ACCESS in your .env to start the server in production.\n',
  );
  throw new Error(
    'UNRESTRICTED_ACCESS cannot be enabled in production. It bypasses path validation and allows unauthenticated reads/writes to any system file.',
  );
}

if (config.UNRESTRICTED_ACCESS) {
  console.warn(
    '\n⚠️  WARNING: UNRESTRICTED_ACCESS is ON. Path validation and tool approval are DISABLED.',
  );
  console.warn(
    '   This is acceptable ONLY for local development. Do NOT deploy this configuration.\n',
  );
}
...
export function getAllowedPaths(): string[] {
  ...
  return config.PATH_ALLOWLIST.split(',')
    .map((p) => p.trim())
    .filter((p) => p.length > 0);
}
...
export function getSafeDirectories(): string[] {
  ...
  return config.SAFE_DIRECTORIES.split(',')
    .map((p) => p.trim())
    .filter((p) => p.length > 0);
}
