...
export class TelegramChannel implements Channel {
  ...
  private getSessionIdForChat(chatId: string, chatType: string): string {
    if (this.isGroupChat(chatType)) {
      ...
      if (!this.sessionIdsPerChat.has(chatId)) {
        this.sessionIdsPerChat.set(chatId, `telegram:${chatId}`);
      }
  ...
  private async textToSpeech(text: string, sessionId: string | undefined): Promise<Buffer | null> {
    try {
      const voiceSettings = sessionId
        ? getVoiceSettings(sessionId)
        : { mode: 'off', ttsProvider: 'openai' };
  ...
  async start(onMessage: (msg: UnifiedMessage) => Promise<void>): Promise<void> {
    ...
    this.bot.use(async (ctx, next) => {
      ...
      if (
        ...
        userId !== config.TELEGRAM_ALLOWED_USER_ID
      ) {
    ...
    this.bot.command('group', async (ctx) => {
      ...
      await ctx.reply('Unknown subcommand. Use `/group` to see current settings.', {
        parse_mode: 'Markdown',
      });
    });
    ...
    this.bot.on('message:voice', async (ctx) => {
      try {
        ...
        if (voiceMode === 'off') {
          await ctx.reply(
            '🔇 Voice messages are disabled. Use `/voice mode transcribe-only` to enable.',
          );
          return;
        }
  ...
  async sendMessage(chatId: string, text: string, sessionId?: string): Promise<void> {
    ...
    const escMdV2 = (s: string) => s.replace(/[_*[\]()~`>#+\-=|{}.!\\]/g, (c) => `\\${c}`);
    ...
    const sendChunk = async (chunk: string): Promise<void> => {
      ...
      try {
        ...
        log.warn(
          `MarkdownV2 send failed, trying plain text: ${e2 instanceof Error ? e2.message : e2}`,
        );
      }
        ...
        if (audioBuffer && audioBuffer.length > 0) {
          ...
          try {
            ...
            try {
              ...
              log.warn(`Failed to clean up temp file: ${tempPath}`, {
