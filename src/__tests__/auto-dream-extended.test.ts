...
} from '../memory/consolidationLock.ts';
...
describe('AutoDream Extended Production Verification', () => {
  ...
  it('4. Database State Synchronization (Orphan Cleanup)', async () => {
    ...
    // Verify stats exist
    let stats: any[] = db
      .prepare('SELECT * FROM fact_stats WHERE session_id = ?')
      .all('test_session_db');
    ...
    stats = db
      .prepare('SELECT fact_text FROM fact_stats WHERE session_id = ?')
      .all('test_session_db') as any[];
