  ...
  describe('Local TTS', () => {
    ...
    it('should throw error if no TTS backend available and fallbackToText=false', async () => {
      try {
        ...
        expect(err instanceof Error ? err.message : String(err)).toContain('local TTS');
      }
    });
    ...
    it('should get TTS backend string', () => {
      ...
      expect(
        backend === 'piper-tts' || backend === 'espeak' || backend === 'text-only (no audio)',
      ).toBe(true);
    });
  });
    ...
    it('should require Ollama when air-gapped', async () => {
      if (config.AIR_GAPPED) {
        ...
        try {
          ...
          console.log('Note: Ollama not running. Install with: ollama serve');
        }
      }
    });
  });
