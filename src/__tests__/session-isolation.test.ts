  ...
  return {
    ...
    getProvider: () => ({
      chat: vi.fn().mockResolvedValue({
        text: 'Mocked child agent response',
        stopReason: 'stop',
      }),
    }),
  };
...
describe('Session Isolation', () => {
  ...
  describe('SessionScopedDB', () => {
    ...
    it('should allow parent DB to read its own session data', () => {
      ...
      parentDB
        .prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)')
        .run(parentSessionId, JSON.stringify({ role: 'user', content: 'test' }));

      // Read it back
      const rows = parentDB
        .prepare('SELECT message_json FROM memory WHERE session_id = ?')
    ...
    it('should allow parent DB to read child session data (child ID starts with parent)', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)').run(
        childSessionId,
        JSON.stringify({ role: 'user', content: 'child task' }),
      );
    ...
    it('should block child DB from reading parent session data', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)').run(
        parentSessionId,
        JSON.stringify({ role: 'user', content: 'parent secret' }),
      );
    ...
    it('should block child DB from reading other unrelated sessions', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)').run(
        otherSessionId,
        JSON.stringify({ role: 'user', content: 'other secret' }),
      );

      // Child DB should NOT be able to read other's memory
      expect(() => {
        childDB.prepare('SELECT message_json FROM memory WHERE session_id = ?').all(otherSessionId);
    ...
    it('should detect SQL injection attempts targeting session_id', () => {
      ...
      expect(() => {
        sessionDB.prepare(
          "SELECT * FROM memory WHERE session_id = ? UNION SELECT * FROM memory WHERE session_id = 'other'",
        );
    ...
    it('should allow parent DB to read child sessions with colon format', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)').run(
        childSessionId,
        JSON.stringify({ role: 'user', content: 'child colon' }),
      );
  ...
  describe('Orchestrator Integration', () => {
    it('should validate session ID format in addUserMessage (requires orchestrator mock setup)', async () => {
      // Import the function
      ...
      expect(() => addUserMessage('', 'test', deps)).toThrow(
        'Session ID must be a non-empty string',
      );
      expect(() => addUserMessage('a'.repeat(256), 'test', deps)).toThrow(
        'Session ID exceeds maximum length',
      );
      expect(() => addUserMessage('invalid!@#', 'test', deps)).toThrow(
        'Session ID contains invalid characters',
      );
    });

    it('should validate session ID format in getHistory (requires orchestrator mock setup)', async () => {
  ...
  describe('Swarm Session Isolation', () => {
    it('should isolate child agent memory from parent', async () => {
      ...
      const result = await swarm.spawnAgent('researcher', 'Investigate the codebase');

      expect(result.sessionId).not.toBe(parentSessionId);

      // Child agent's memory should be recorded under the child session ID only
      const childRows = db
        .prepare('SELECT COUNT(*) as count FROM memory WHERE session_id = ?')
        .get(result.sessionId) as { count: number };
      expect(childRows.count).toBeGreaterThan(0);

      // Parent session should have no memory written by the child
      const parentRows = db
        .prepare('SELECT COUNT(*) as count FROM memory WHERE session_id = ?')
        .get(parentSessionId) as { count: number };
      expect(parentRows.count).toBe(0);

      // Child session should not be able to read parent memory (isolation enforced)
      const childDB = createSessionDB(result.sessionId, parentSessionId);
      expect(() => {
        childDB
          .prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)')
