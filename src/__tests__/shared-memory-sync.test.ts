...
describe('shared memory sync', () => {
  it('bootstraps shared memory contract files', () => {
    ...
    expect(
      fs.existsSync(path.join(workspaceRoot, '.ai_memory', 'projects', 'gravityclaw.json')),
    ).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.ai_memory', 'projects', 'aegis-ai.json'))).toBe(
      true,
    );
  });
