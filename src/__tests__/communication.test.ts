...
describe('Agent-to-Agent Communication', () => {
  beforeEach(() => {
    ...
    db.prepare(
      "DELETE FROM messages WHERE from_session_id LIKE 'test:comm:%' OR to_session_id LIKE 'test:comm:%'",
    ).run();
    db.prepare(
      "DELETE FROM permissions WHERE session_id LIKE 'test:comm:%' OR target_session_id LIKE 'test:comm:%'",
    ).run();
    db.prepare("DELETE FROM sessions WHERE id LIKE 'test:comm:%'").run();
    db.prepare(
      "DELETE FROM agent_swarms WHERE parent_session_id LIKE 'test:comm:%' OR child_session_id LIKE 'test:comm:%'",
    ).run();
  ...
  describe('error handling', () => {
    ...
    it('should handle concurrent message sends gracefully', async () => {
      ...
      const promises = Array.from({ length: 5 }, (_, i) =>
        Promise.resolve(
          AgentMessaging.sendMessage(childSessionId, parentSessionId, `message ${i + 1}`),
        ),
      );
  ...
  describe('integration scenarios', () => {
    it('should support research → writer agent flow', () => {
      ...
      // Cleanup
      db.prepare(
        "DELETE FROM messages WHERE from_session_id LIKE 'session:%' OR to_session_id LIKE 'session:%'",
      ).run();
