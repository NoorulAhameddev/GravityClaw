...
function seedDb(dbPath: string, sessionId: string): void {
  ...
  db.prepare('INSERT INTO memory (session_id, timestamp, message_json) VALUES (?, ?, ?)').run(
    sessionId,
    '2026-05-31T10:00:00.000Z',
    JSON.stringify({ role: 'user', content: 'Investigate vault sync drift' }),
  );
  db.prepare('INSERT INTO memory (session_id, timestamp, message_json) VALUES (?, ?, ?)').run(
    sessionId,
    '2026-05-31T10:05:00.000Z',
    JSON.stringify({ role: 'assistant', content: 'Root cause identified and sync path fixed.' }),
  );
  db.close();
}
...
describe('vault sync', () => {
  it('writes the latest session into the vault', () => {
    ...
    fs.writeFileSync(
      path.join(vaultRoot, 'vault-context.md'),
      '> Last sync: 2026-05-15 (old)\n',
      'utf8',
    );
    ...
    const archive = fs.readFileSync(
      path.join(vaultRoot, '9-Decisions', 'sessions', 'gravityclaw-test-session-1.md'),
      'utf8',
    );
  ...
  it('does not duplicate the same session entry on repeated sync', () => {
    ...
    const dailyNote = fs.readFileSync(path.join(vaultRoot, '1-Daily', '2026-05-31.md'), 'utf8');
    const occurrences =
      dailyNote.match(/\[\[9-Decisions\/sessions\/gravityclaw-test-session-repeat\]\]/g) ?? [];
