...
async function testPruningSettings() {
  ...
  console.log(
    'History length before pruning:',
    (db.prepare('SELECT count(*) as count FROM memory WHERE session_id = ?').get(sessionId) as any)
      .count,
  );
...
async function testNonMutatingOverrides() {
  ...
  try {
    ...
    console.log(
      `Provider creation failed as expected: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
