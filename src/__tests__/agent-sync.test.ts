  ...
  it('imports OpenCode, Claude JSONL, and Codex metadata into one vault state', () => {
    ...
    fs.writeFileSync(
      ...
      [
        JSON.stringify({
          sessionId: 'claude-1',
          cwd: 'd:\\Projects\\GravityClaw',
          timestamp: '2026-05-10T11:22:52.442Z',
          type: 'user',
          message: { role: 'user', content: [{ type: 'text', text: 'Investigate bug' }] },
        }),
        JSON.stringify({
          sessionId: 'claude-1',
          cwd: 'd:\\Projects\\GravityClaw',
          timestamp: '2026-05-10T11:22:57.850Z',
          type: 'assistant',
    ...
    const state = JSON.parse(
      fs.readFileSync(path.join(vaultRoot, '9-Auto', 'agent-sync-state.json'), 'utf8'),
    );
