...
describe('Rate Limiter', () => {
  ...
  describe('HTTP Endpoint Limiting', () => {
    it('should apply the http category (not tool) to http-prefixed endpoints', () => {
      // Regression: "http:api" previously fell through to the "tool" category (burst=3),
      // which throttled dashboard API polling to 3 requests/minute.
      const apiSession = 'api:::1';
      rateLimiter.resetSessionLimits(apiSession);

      // http category burst is 30 — all of these should be allowed
      for (let i = 0; i < 20; i++) {
        const status = rateLimiter.checkRateLimit(apiSession, 'http:api');
        expect(status.allowed).toBe(true);
      }
