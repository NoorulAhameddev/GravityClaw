...
describe('Session Settings', () => {
  beforeEach(() => {
    ...
    db.prepare('DELETE FROM session_settings WHERE session_id IN (?, ?)').run(
      testSessionId1,
      testSessionId2,
    );
  });
  ...
  describe('setSessionSettings', () => {
    ...
    it('should update all messages in session', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json, settings) VALUES (?, ?, ?)').run(
        testSessionId1,
        JSON.stringify({ role: 'user', content: 'test1' }),
        '{}',
      );
      db.prepare('INSERT INTO memory (session_id, message_json, settings) VALUES (?, ?, ?)').run(
        testSessionId1,
  ...
  describe('deleteSession', () => {
    it('should delete all messages for session', () => {
      ...
      const rows = db.prepare('SELECT * FROM memory WHERE session_id = ?').all(testSessionId1);
  ...
  describe('listSessions', () => {
    ...
    it('should list all unique session IDs', () => {
      ...
      db.prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?)').run(
        testSessionId2,
        JSON.stringify({ role: 'user', content: 'test2' }),
      );
