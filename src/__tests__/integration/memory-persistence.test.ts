...
describe('Memory Persistence Integration Tests', () => {
  ...
  describe('Fact CRUD Operations', () => {
    it('should create and retrieve facts', async () => {
      ...
      rows.forEach((row, idx) => {
        expect(row.category).toBe(mockMemoryFacts[idx]!.category.toLowerCase());
    ...
    it('should increment access count on fact retrieval', async () => {
      ...
      const { getFactHash, touchFactAccess } = await import('../../memory/markdown.ts');
    ...
    it('should delete facts properly', async () => {
      ...
      const rowsBefore = db
        .prepare(`SELECT COUNT(*) as count FROM fact_stats WHERE session_id = ?`)
        .get(testSessionId) as { count: number };
      expect(rowsBefore.count).toBeGreaterThan(0);

      db.prepare(`DELETE FROM fact_stats WHERE session_id = ?`).run(testSessionId);
    ...
    it('should handle duplicate facts', async () => {
      ...
      const { getFactHash } = await import('../../memory/markdown.ts');
      const factHash = getFactHash(fact);
    ...
    it('should track fact access timestamps', async () => {
      ...
      const row = db
        .prepare(`SELECT last_accessed FROM fact_stats WHERE session_id = ? LIMIT 1`)
  ...
  describe('Knowledge Graph Entity Operations', () => {
    it('should create entities with properties', async () => {
      for (const entity of mockEntities) {
        const id = createTestEntity(testSessionId, entity.name, entity.type, entity.properties);
        expect(id).toBeGreaterThan(0);
      }

      const rows = db
        .prepare(`SELECT name, type FROM entities WHERE session_id = ? ORDER BY id ASC`)
    ...
    it('should update entity properties', async () => {
      ...
      db.prepare(`UPDATE entities SET properties = ? WHERE id = ?`).run(
        JSON.stringify(newProperties),
        id,
      );

      const row = db.prepare(`SELECT properties FROM entities WHERE id = ?`).get(id) as
    ...
    it('should handle entity uniqueness constraint', async () => {
      ...
      const id1 = createTestEntity(testSessionId, entity.name, entity.type, entity.properties);
      const id2 = createTestEntity(testSessionId, entity.name, entity.type, {
  ...
  describe('Knowledge Graph Relationship Operations', () => {
    it('should create relationships between entities', async () => {
      ...
      // Create relationship
      createTestRelationship(testSessionId, mockEntities[0]!.name, mockEntities[1]!.name, 'uses');

      const rows = db
        .prepare(`SELECT relation_type FROM relationships WHERE session_id = ?`)
  ...
  describe('Memory Data Integrity', () => {
    it('should maintain consistency between facts and entities', async () => {
      ...
      const entities = db
        .prepare(`SELECT COUNT(*) as count FROM entities WHERE session_id = ?`)
        .get(testSessionId) as { count: number };
