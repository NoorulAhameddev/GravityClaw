...
describe('Channel → Agent → Tool Integration', () => {
  ...
  describe('Agent Tool Execution', () => {
    ...
    it('should handle multiple tool calls in sequence', async () => {
      ...
      for (const tool of tools) {
        insertTestMessage(testSessionId, 'assistant', `Calling ${tool}...`);
    ...
    it('should handle tool errors gracefully', async () => {
      ...
      insertTestMessage(testSessionId, 'tool-error', 'Tool execution failed: Permission denied');

      const history = getSessionHistory(testSessionId);
      const errorMsg = history.find((m) => m.role === 'tool-error');
  ...
  describe('Multi-Turn State Management', () => {
    it('should maintain context across turns', async () => {
      ...
      insertTestMessage(testSessionId, 'user', 'Tell me about normalization in database design');
      insertTestMessage(
        testSessionId,
        'assistant',
  ...
  describe('Error Handling in Message Flow', () => {
    ...
    it('should handle concurrent message insertions', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(Promise.resolve(insertTestMessage(testSessionId, 'user', `Message ${i}`)));
      }
  ...
  describe('Agent Iteration Handling', () => {
    it('should track iteration count in multi-turn agent loop', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        insertTestMessage(testSessionId, 'agent-iteration', `Iteration ${i + 1}`);
        insertTestMessage(testSessionId, 'assistant', `Progress: step ${i + 1}`);
      }

      const history = getSessionHistory(testSessionId);
      const iterations = history.filter((m) => m.role === 'agent-iteration');
    ...
    it('should handle agent reaching max iterations', async () => {
      ...
      insertTestMessage(testSessionId, 'system', 'Max iterations reached, stopping agent');

      const history = getSessionHistory(testSessionId);
      const iterations = history.filter((m) => m.role === 'assistant');
  ...
  describe('Tool Call Tracking', () => {
    it('should track total tool calls in conversation', async () => {
      ...
      const toolCalls = ['read_file', 'write_file', 'execute_command'];
    ...
    it('should record tool call failures', async () => {
      ...
      for (const tool of failedTools) {
        ...
        insertTestMessage(testSessionId, 'tool-error', `${tool} failed`);
      }
  ...
  describe('Channel-Specific Responses', () => {
    it('should format response for Telegram', async () => {
      ...
      insertTestMessage(telegramSession, 'assistant', 'Welcome to Gravity Claw! 👋');
    ...
    it('should format response for WhatsApp', async () => {
      ...
      insertTestMessage(whatsappSession, 'assistant', 'Hi there! How can I help? 😊');
  ...
  describe('Response Completion Handling', () => {
    it('should mark conversation as complete when agent stops', async () => {
      ...
      const completionMsg = history.find(
        (m) =>
          m.role === 'system' && typeof m.content === 'string' && m.content.includes('completed'),
      );
    ...
    it('should handle partial responses', async () => {
      ...
      insertTestMessage(testSessionId, 'system', 'Partial: 50% complete');
