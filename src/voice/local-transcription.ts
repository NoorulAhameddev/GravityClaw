...
function isWhisperAvailable(): boolean {
  try {
    ...
    const result = execSync('where whisper > nul 2>&1 || which whisper > /dev/null 2>&1', {
      timeout: 2000,
      stdio: 'pipe',
    });
    return true;
...
export async function localTranscribe(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      log.debug(`Running: whisper ${args.join(' ')}`);
...
export async function downloadWhisperModel(model: string = 'base'): Promise<void> {
  ...
  try {
    execFileSync('whisper-cpp-download-model.sh', [model], { timeout: 300000, stdio: 'inherit' }); // 5 minute timeout
