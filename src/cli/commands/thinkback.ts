...
export async function thinkbackCommand(
  options: {
    year?: number;
    play?: boolean;
  } = {},
...
async function showYearInReview(year: number): Promise<void> {
  ...
  if (review.topTools.length > 0) {
    const rows = review.topTools.slice(0, 8).map((t) => [t.name, t.count.toLocaleString()]);
  ...
  if (review.topModels.length > 0) {
    const rows = review.topModels
      .slice(0, 5)
      .map((m) => [m.name.slice(0, 30), `$${m.cost.toFixed(2)}`]);
