...
function getMemoryStats(): {
  ...
  const sessionResult = db
    .prepare(`SELECT COUNT(DISTINCT session_id) as count FROM memory`)
    .get() as { count: number };

  // Get total message count
  const messageResult = db.prepare(`SELECT COUNT(*) as count FROM memory`).get() as {
    count: number;
  };
...
export async function statsCommand(
  options: { period?: string; verbose?: boolean } = {},
): Promise<void> {
  ...
  try {
    ...
    const usageStats: UsageStats = getUsageStats(
      undefined,
      period !== 'all'
        ? new Date(
            Date.now() -
    ...
    try {
      const oldestResult = db.prepare(`SELECT MIN(timestamp) as oldest FROM memory`).get() as {
        oldest: string | null;
      };
