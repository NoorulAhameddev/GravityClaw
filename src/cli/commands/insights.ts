...
function analyzeSessions(): {
  ...
  return {
    ...
    avgMessagesPerSession: sessionResult.count > 0 ? messageResult.count / sessionResult.count : 0,
    sessionsByDay,
    messagesByHour,
  };
}
...
export async function insightsCommand(
  ...
  const usagePeriod = period === 'day' ? 1 : period === 'week' ? 7 : period === 'month' ? 30 : 90;
  ...
  if (usageStats.models.length > 0) {
    ...
    const rows = usageStats.models
      .slice(0, 5)
      .map((m) => [m.model.slice(0, 25), m.calls.toLocaleString(), `$${m.cost.toFixed(2)}`]);
  ...
  if (toolUsage.length > 0) {
    ...
    const rows = toolUsage.slice(0, 8).map((t) => [t.tool, t.count.toLocaleString()]);
