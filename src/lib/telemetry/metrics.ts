...
export const apiErrorsCounter = getOrCreateCounter('api_errors_total', 'Total API errors', '1');
...
export const agentRunsCounter = getOrCreateCounter('agent_runs_total', 'Total agent runs', '1');
...
export const toolCallsCounter = getOrCreateCounter('tool_calls_total', 'Total tool calls', '1');
...
export const llmCallsCounter = getOrCreateCounter('llm_calls_total', 'Total LLM API calls', '1');
...
export function recordLlmCall(
  ...
  if (tokens) {
    meter
      .createCounter('llm_tokens_total', { description: 'Total LLM tokens' })
      .add((tokens.prompt ?? 0) + (tokens.completion ?? 0), {
        provider,
        model,
        token_type: 'total',
      });
...
export function recordWorkerJob(
  ...
  if (delayMs !== undefined) {
    meter
      .createHistogram('worker_job_delay', { description: 'Worker job queue delay' })
      .record(delayMs, {
