...
export async function runForkedAgent(options: ForkedAgentOptions): Promise<ForkedAgentResult> {
  ...
  for (let i = 0; i < maxIterations; i++) {
    const response: ClaudeResponse = await callClaude(sessionId, toolDefs, {}, orchestratorDeps);
  ...
  log.debug(
    `Forked agent completed: ${sessionId}, ` + `${result.totalUsage.totalTokens} tokens used`,
  );
