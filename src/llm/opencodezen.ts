...
export class OpenCodeZenProvider implements LLMProvider {
  ...
  constructor(
    apiKey: string,
    defaultModel: string = 'minimax-m2.5-free',
    ...
    log.info(
      `OpenCodeZen provider initialized with model: ${defaultModel} and baseURL: ${baseURL}`,
    );
  }
  ...
  async chat(
    ...
    log.debug(
      `Calling OpenCodeZen — model: ${model}, messages: ${messages.length}, tools: ${toolDefinitions.length}`,
    );
