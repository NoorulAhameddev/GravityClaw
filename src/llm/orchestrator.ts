...
import { getSessionSettings, ensureSession } from '../session.ts';
...
function extractFacts(content: string): string[] {
  ...
  if (
    ...
    lowerContent.includes('[system]') ||
    (lowerContent.startsWith('my ') &&
      lowerContent.includes('is ') &&
      (lowerContent.includes(' to ') || lowerContent.includes(' and ')))
  ) {
...
function validateSessionId(sessionId: string): void {
  ...
  if (!/^[a-zA-Z0-9\-_:]+$/.test(sessionId)) {
    throw new Error(
      'Session ID contains invalid characters. Only alphanumeric, hyphens, underscores, and colons are allowed.',
    );
  }
}
...
export function addUserMessage(
  ...
  const row = deps.db
    .prepare('INSERT INTO memory (session_id, message_json) VALUES (?, ?) RETURNING id')
    .get(sessionId, JSON.stringify(msg)) as { id: number } | undefined;
...
export function addToolResult(
  ...
  deps.db
    ...
    .run(sessionId, JSON.stringify(msg));
...
export async function callClaude(
  ...
  if (shouldCompact) {
    try {
      ...
      if (mcResult && mcResult.messages.length > 0) {
        ...
        log.debug(
          `Applied microcompact: ${mcResult.compactedCount} tools compacted, ~${mcResult.tokensFreed} tokens freed`,
        );
      }
    } catch (e) {
  ...
  const attachmentContext =
    promptContext?.attachmentContext ?? getRecentAttachmentContext(sessionId);
...
`;
  ...
  if (systemPromptContent.length > MAX_SYSTEM_LENGTH) {
    log.warn(
      `System prompt exceeded ${MAX_SYSTEM_LENGTH} chars, trimming from ${systemPromptContent.length}`,
    );
  ...
  if (sessionSettings.provider || sessionSettings.model) {
    ...
    telemetryLogger.info('llm session override', {
  ...
  try {
    ...
    if (useStream) {
      ...
      response = await withRetrySimple(async () =>
        provider.chat(messagesWithSystem, toolDefinitions),
      );
    } else {
