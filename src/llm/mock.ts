...
const RESPONSES: Record<string, string> = {
  default:
    "I'm a mock agent responding without an LLM API. I can help with file operations, running shell commands, and other tasks.",
  hello: "Hello! I'm running in mock mode for testing.",
  'how are': "I'm working perfectly in mock test mode!",
  'what is':
    'In mock mode, I provide simple responses. The tool filtering and safety features are fully functional.',
...
export class MockProvider implements LLMProvider {
  ...
  countTokens(messages: ChatCompletionMessageParam[]): number {
    return messages.reduce(
      (acc, m) => acc + (typeof m.content === 'string' ? m.content.length : 50),
      0,
    );
  }
