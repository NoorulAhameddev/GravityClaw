...
export function getTaskQueue(): TaskQueue {
  if (!queueInstance) {
    if (config.QUEUE_ENABLED && config.REDIS_URL) {
      log.warn(
        '⚠️ BullMQ backend not yet implemented — REDIS_URL is configured but Redis-backed queue is unavailable. Falling back to SQLite queue.',
      );
    }
...
export async function enqueueToolTaskWithTrace(
...
export function shouldQueueTool(toolName: string, hasQueueMetadata: boolean): boolean {
