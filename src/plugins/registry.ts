...
export class PluginRegistry {
  ...
  getPluginsByTrait<T extends PluginTrait = PluginTrait>(
  ...
  async install(source: string): Promise<PluginManifest> {
    throw new Error('Plugin installation is not yet implemented.');
  }
  ...
  list(): PluginManifest[] {
    return Array.from(this.plugins.values()).map((p) => ({
      ...
      author: Array.isArray(p.metadata.author)
        ? p.metadata.author.join(', ')
        : p.metadata.author || 'unknown',
      license: p.metadata.license || 'MIT',
      entry: '', // Unknown at runtime
      tools: [], // Not part of current plugin metadata
      permissions: [], // Not part of current plugin metadata
      configSchema: {}, // Config schema not explicitly part of current Plugin metadata
    }));
  }
