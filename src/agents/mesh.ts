...
- Return ONLY valid JSON, no additional text`;
        ...
        [
          {
            ...
            content:
              'You are a JSON-generating task decomposition system. Always respond with valid JSON only.',
          },
          ...history,
        ],
        [],
    ...
    const hasCycleDFS = (taskId: string): boolean => {
      ...
      for (const depId of task.dependsOn) {
        if (!visited.has(depId)) {
          ...
          // Cycle detected
          errors.push(
            `Cycle detected: Task ${taskId} depends on Task ${depId} which eventually depends on Task ${taskId}`,
          );
          return true;
        }
      }
  ...
  private async executeTask(
    ...
    try {
      ...
      const result = await runWithConcurrencyLimit(taskSessionId, () =>
        runAgent({
          message: prompt,
          sessionId: taskSessionId,
          maxIterations: 3,
  ...
  async execute(
    ...
    log.info(
      `Starting workflow execution: ${workflowId} with maxParallelTasks=${this.config.maxParallelTasks}`,
    );
