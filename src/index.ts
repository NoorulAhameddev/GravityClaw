...
import {
  isHeartbeatTask,
  isHeartbeatEnabledForSession,
  markHeartbeatRun,
  isHeartbeatResponseNoteworthy,
...
async function main() {
  ...
  registerTaskExecutionHandler(async (taskId, sessionId, prompt) => {
    ...
    if (isHeartbeatTask(taskId)) {
      ...
      const result = await runWithConcurrencyLimit(sessionId, () =>
        runAgent({
          message: prompt,
          sessionId,
          dependencies: {
      ...
      if (isHeartbeatResponseNoteworthy(result.text)) {
        await router.sendProactiveToSession(
          sessionId,
          `💓 **Heartbeat Update**\n\n${result.text.trim()}`,
        );
      }
  ...
  const recommendationsRuntime = startDailyRecommendations((sessionId, text) =>
    router.sendProactiveToSession(sessionId, text),
  );
  const autoDreamRuntime = startAutoDreamScheduler();
