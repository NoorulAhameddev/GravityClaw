...
router.get('/health', healthAuthMiddleware, (req, res) => {
  ...
  try {
    ...
    try {
      const result = db
        .prepare("SELECT COUNT(*) as count FROM memory WHERE type = 'fact'")
        .get() as { count: number };
      factCount = result?.count || 0;
    } catch (err) {
      log.warn('Failed to fetch fact count in health check', { error: err });
    }
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    try {
      ...
      log.warn('Database readiness check failed', { error: err });
    }
    ...
    try {
      ...
      log.warn('LLM provider readiness check failed', { error: err });
    }
