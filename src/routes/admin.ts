      ...
      period === '24h'
        ...
        : period === '7d'
          ...
          : period === '30d'
            ...
            : '';
    const count = (
      db.prepare(`SELECT COUNT(*) as count FROM sessions ${periodSql}`).get() as { count: number }
    ).count;
...
router.get('/usage', authMiddleware, async (req, res) => {
  try {
    ...
    allTime.models.forEach((m: { model: string; calls: number; tokens: number; cost: number }) => {
