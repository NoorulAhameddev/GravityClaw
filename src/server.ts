...
// CSRF protection is implicitly handled by the fact that the API requires an `x-api-key`
// header (or a Bearer token) for authentication. Browsers do not automatically attach
// these headers to cross-origin requests, so CSRF attacks are not possible against
...
app.get('/api/usage', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    allTime.models.forEach((m) => {
      modelsObj[m.model] = { calls: m.calls, tokens: m.tokens, cost: m.cost };
    });
...
app.get('/api/heartbeats', authMiddleware, (req, res) => {
  try {
    const heartbeats = db
      .prepare('SELECT * FROM heartbeat_tasks ORDER BY created_at DESC LIMIT 100')
      .all();
