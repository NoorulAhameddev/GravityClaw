...
async function getConfig(): Promise<CalendarConfig> {
  const credentialsPath =
    config.GOOGLE_CREDENTIALS_PATH || (await getSecret('GOOGLE_CREDENTIALS_PATH'));
...
async function getCalendarClient(): Promise<calendar_v3.Calendar> {
  ...
  if (!cfg.credentialsPath && !cfg.apiKey) {
    throw new Error(
      'Google Calendar not configured. Set GOOGLE_CALENDAR_API_KEY or GOOGLE_CREDENTIALS_PATH in environment.',
    );
  }
