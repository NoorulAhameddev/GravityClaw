...
async function getNotionClient(): Promise<Client> {
  ...
  if (!apiKey) {
    throw new Error(
      'Notion API key not configured. Set NOTION_API_KEY in environment or add to secrets store.',
    );
  }
...
export const notionCreatePageTool: Tool = {
  ...
  async execute(input: Record<string, unknown>): Promise<string> {
    try {
      ...
      const { parentPageId, title, content } = input as {
        parentPageId?: string;
        title?: string;
      ...
      const response = (await notion.pages.create(createParams as never)) as {
        id: string;
        url?: string;
      };
...
export const notionReadPageTool: Tool = {
  ...
  async execute(input: Record<string, unknown>): Promise<string> {
    try {
      ...
      const { pageId, maxDepth = 3 } = input as {
        pageId?: string;
        maxDepth?: number;
      };
      ...
      const pageTitle =
        (page.properties?.title as { title?: Array<{ plain_text?: string }> })?.title?.[0]
          ?.plain_text ||
        (page.properties?.Name as { title?: Array<{ plain_text?: string }> })?.title?.[0]
          ?.plain_text ||
        'Untitled';
      ...
      const parsedContent = blocks
        .map((block) => parseBlockContent(block))
        .filter(Boolean)
        .join('\n');
