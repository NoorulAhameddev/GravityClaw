...
import { getGroupSettings, DANGEROUS_TOOLS } from '../../groups/index.ts';
...
export const shutdownSystemTool: Tool = {
  name: 'shutdownSystem',
  description:
    'Shutdown or restart the Gravity Claw process (Admin only, requires explicit approval)',
  inputSchema: {
    type: 'object',
    properties: {
      reason: {
  ...
  async execute(input: Record<string, unknown>): Promise<string> {
    ...
    if (input.__groupId && input.__platform) {
      ...
      const isAdmin = isGroupAdmin(
        input.__platform as string,
        input.__groupId as string,
        input.__userId as string,
      );
      if (!isAdmin) {
    ...
    log.warn(
      `SYSTEM SHUTDOWN REQUESTED by user ${input.__userId}: ${reason} (restart: ${restart})`,
    );
