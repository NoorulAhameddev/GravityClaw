...
import type { GraphEntity, GraphRelationship } from '../../memory/graph.ts';
...
export const listEntitiesTool: Tool = {
  ...
  async execute(input: Record<string, unknown>): Promise<string> {
    try {
      ...
      let query =
        'SELECT id, session_id, name, type, properties, access_count, last_accessed FROM entities WHERE session_id = ?';
      const params: unknown[] = [sessionId];
...
function getEntityRelationshipCount(entityId: number): number {
  try {
    const result = db
      .prepare('SELECT COUNT(*) as count FROM relationships WHERE from_id = ? OR to_id = ?')
