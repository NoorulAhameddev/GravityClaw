...
export const getRateLimitStatusTool: Tool = {
  ...
  async execute(input: Record<string, unknown>): Promise<string> {
    ...
    const response = {
      ...
      message: `You have ${Math.floor(status.tokensAvailable)} requests available out of ${
        status.limit.requestsPerMinute
      } per minute.`,
    };
...
export const updateRateLimitsTool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      requestsPerMinute: {
        type: 'number',
        description: 'New rate limit in requests per minute (must be >= 1 and <= default limit)',
      },
    },
...
export const getRateLimitHistoryTool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      toolName: {
        type: 'string',
        description: "Filter by specific tool name (e.g., 'save_fact', 'text_to_speech')",
      },
    },
    required: [],
  },
