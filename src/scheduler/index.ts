...
function calculateNextRun(cronExpression: string): Date | null {
  try {
    ...
    const cronDate = (
      schedule as unknown as { nextDate: () => { toDate: () => Date } }
    ).nextDate?.();
    schedule.stop();
...
async function executeTask(task: ScheduledTask): Promise<void> {
  ...
  try {
    ...
    log.warn(
      `Scheduled task failed (no handler/channel): ${task.name} (id: ${task.id}, error: ${error instanceof Error ? error.message : String(error)})`,
    );
  }
}
...
function startCronJob(task: ScheduledTask): void {
  ...
  try {
    ...
    log.error(
      `Failed to start cron job - id: ${task.id}, cron: ${task.cronExpression}, error: ${error}`,
    );
  }
}
...
function loadEnabledTasks(): void {
  try {
    const tasks = db.prepare('SELECT * FROM scheduled_tasks WHERE enabled = 1').all() as Record<
      string,
      unknown
    >[];
...
export const listTasksTool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      all_sessions: {
        type: 'boolean',
        description: 'If true, list tasks from all sessions (default: false, only current session)',
      },
    },
    required: [],
  },
