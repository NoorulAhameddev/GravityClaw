...
function isContextuallyRelevant(memory: string, query: string, similarity: number): boolean {
...
export async function retrieveRelevantMemories(
  ...
  const ranked: RetrievedMemory[] = raw
    ...
    .map((r) => {
      const memType = ((r as SemanticSearchResult & { memoryType?: 'fact' | 'conversation' })
        .memoryType || 'conversation') as 'fact' | 'conversation';
      return {
        id: r.id,
  ...
  const result = fitToBudget(deduped, maxChars, limit, maxPerItem) as RetrievedMemory[] & {
    retrievalMode?: RetrievalMode;
  };
...
export function formatRelevantMemories(memories: RetrievedMemory[]): string {
  ...
  for (const memory of memories) {
    const roleLabel =
      memory.role === 'user' ? 'User' : memory.role === 'assistant' ? 'Assistant' : 'System';
