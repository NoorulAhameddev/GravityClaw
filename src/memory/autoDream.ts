...
function getAllSessionIds(): string[] {
  const rows = db
    .prepare(`SELECT DISTINCT session_id FROM memory ORDER BY timestamp DESC LIMIT 100`)
...
export async function executeAutoDream(): Promise<void> {
  ...
  if (sinceScanMs < SCAN_SESSIONS_INTERVAL_MS) {
    log.debug(`Scan throttle: last scan was ${Math.round(sinceScanMs / 1000)}s ago`);
    return;
  }
  ...
  try {
    ...
    for (const sessionId of sessionIds) {
      try {
        ...
        const responseText = result.messages.map((m) => m.content).join('\n');

        const factLines = responseText.split('\n').filter((line) => line.startsWith('- ['));
        ...
        if (consolidatedFacts.length > 0) {
          ...
          log.info(
            `Consolidated memory for session ${sessionId}: reduced from ${facts.length} to ${consolidatedFacts.length} facts`,
          );
          consolidatedCount++;
        }
      } catch (err) {
    ...
    log.info(`AutoDream completed — consolidated ${consolidatedCount} sessions`);
  } catch (e) {
