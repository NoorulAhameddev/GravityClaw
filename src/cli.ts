...
async function run(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      case 'sessions':
        await sessionsCommand(args.subcommand, args.positional[0]);
        break;

      case 'bridge':
        const { bridgeCommand } = await import('./cli/commands/bridge.ts');
        await bridgeCommand(
          args.subcommand ? [args.subcommand, ...args.positional] : args.positional,
        );
        break;
      ...
      case 'skills':
        await skillsCommand(args.subcommand, args.positional);
        break;
      ...
      case 'plugin':
        await pluginCommand(args.subcommand, args.positional);
        break;
