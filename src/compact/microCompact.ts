...
export async function applyMicrocompact(
  ...
  if (compactedCount > 0) {
    log.debug(`Microcompact: ${compactedCount} tools compacted, ~${tokensFreed} tokens freed`);
  }
...
export function shouldRunMicrocompact(messageCount: number, lastApiCallTokens?: number): boolean {
