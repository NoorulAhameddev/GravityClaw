...
export function getRecommendationsStatus(sessionId: string): {
  ...
  return {
    ...
    lastSentDate:
      typeof settings.recommendationsLastSentDate === 'string'
...
export function markRecommendationSent(
  sessionId: string,
  suggestions: string[],
  dateKey = getDateKey(),
): void {
...
function getHeuristicRecommendations(profile: RecommendationsProfile): string[] {
  ...
  if (usageCmd && usageCmd.count >= 2) {
    suggestions.push(
      'You run /usage often. I can include daily cost highlights in your evening recap.',
    );
  }
  ...
  if (profile.commonQueries.length >= 3) {
    suggestions.push(
      'You revisit similar queries. I can save a short knowledge summary for faster answers.',
    );
  }

  if (suggestions.length === 0) {
    suggestions.push(
      'I can set up heartbeat prompts so you get proactive updates only when something important changes.',
    );
  }
