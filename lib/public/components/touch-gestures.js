...
class SwipeDetector {
  ...
  handleTouchEnd(e) {
    ...
    const minDistance = GESTURE_CONFIG.swipe.minDistance * this.prefs.get('swipeSensitivity', 1.0);
...
class PullToRefreshDetector {
  ...
  isScrolledToTop() {
    const scrollable =
      this.element.querySelector('[data-scrollable]') ||
  ...
  handleTouchStart(e) {
    if (
      !this.prefs.get('pullToRefreshEnabled') ||
...
class LongPressDetector {
  ...
  handleTouchStart(e) {
    if (!this.prefs.get('longPressEnabled') || GESTURE_CONFIG.longPress.enabled === false) return;
...
class PinchZoomDetector {
  ...
  handleTouchStart(e) {
    if (!this.prefs.get('pinchZoomEnabled') || GESTURE_CONFIG.pinchZoom.enabled === false) return;
...
class TouchGestureManager {
  ...
  handleSwipe(direction) {
    ...
    const currentPage =
      document.querySelector('[data-page].active')?.dataset.page ||
