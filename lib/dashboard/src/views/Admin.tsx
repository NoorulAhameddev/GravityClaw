    ...
    <div className="p-8 space-y-6">
      <div className="flex items-start gap-5 p-6 rounded-2xl bg-surface border border-border">
        ...
        <div className="space-y-1 min-w-0 flex-1">
          ...
          <p className="text-sm text-muted">
            Manage platform group permissions, tool access controls, and agent authorization
            policies.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Total Enabled Tools',
            value: groups.reduce((a, g) => a + g.enabledToolCount, 0),
            icon: Wrench,
            color: 'text-green-400',
          },
          {
            label: 'Total Disabled',
            value: groups.reduce((a, g) => a + g.disabledToolCount, 0),
            icon: Wrench,
            color: 'text-red-400',
          },
        ].map((s) => (
        ...
        {loading && groups.length === 0 ? (
          ...
          <div className="flex flex-col items-center gap-2 py-16 text-red-400">
            ...
            <p className="text-xs text-muted">
              This panel requires an active WebSocket connection and tool access.
            </p>
          </div>
        ) : groups.length === 0 ? (
