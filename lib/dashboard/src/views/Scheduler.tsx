...
const features = [
  {
    ...
    desc: 'Exponential backoff and intelligent retry strategies with configurable limits',
  },
  {
    icon: '🔍',
    title: 'Failure Auto-Diagnosis',
    desc: 'Automatically diagnose, categorize, and report task failures',
  },
  {
    icon: '💀',
    title: 'Dead-Letter Recovery',
    desc: 'Recover and replay failed tasks from dead-letter queue',
  },
  {
    icon: '📊',
    title: 'SLA Tracking',
    desc: 'Monitor execution times against SLA targets and alert on violations',
  },
];
      ...
      <div className="flex items-start gap-5 p-6 rounded-2xl bg-surface border border-border">
        ...
        <div className="space-y-2 min-w-0">
          ...
          <p className="text-sm text-muted leading-relaxed">
            Manages recurring jobs, cron schedules, retries with exponential backoff, failure
              ...
              {loading && tasks.length === 0 ? (
                ...
                filteredTasks.map((t) => (
                  <tr key={t.name} className="hover:bg-surface2/30 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <code className="px-2 py-0.5 bg-surface2 rounded text-xs font-mono text-accent">
                        {t.cron_expression}
                      </code>
                    </td>
                    <td className="px-4 py-3">
                      <code className="text-xs font-mono text-muted">{t.session_id}</code>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={cn(
