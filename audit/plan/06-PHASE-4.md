...
const RBAC_MATRIX: Record<Role, Permission[]> = {
  ...
  user: ['session:read', 'session:write', 'tools:execute', 'users:read', 'settings:read'],
    ...
    healthcheck:
      test:
        [
          ...
          'node',
          '-e',
          "fetch('http://localhost:3000/api/live').then(r=>process.exit(r.ok?0:1))",
        ]
      interval: 15s
      timeout: 5s
      retries: 3
