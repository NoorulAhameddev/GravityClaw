...
class ToolBenchmark {
  ...
  private generateSuite(): BenchmarkSuite {
    ...
    const averageTime = this.results.reduce((sum, r) => sum + r.avg, 0) / this.results.length;
  ...
  private printSummary(suite: BenchmarkSuite): void {
    ...
    if (suite.summary.failCount > 0) {
      ...
      suite.tools
        ...
        .forEach((t) => {
          console.log(
            `  ${t.toolName}: ${t.avg.toFixed(3)}ms (${(t.avg - this.targetLatency).toFixed(3)}ms over target)`,
          );
        });
    }
