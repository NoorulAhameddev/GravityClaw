...
function getMasterKey(): string {
  ...
  if (!key) {
    logger.error(
      '❌ MASTER_KEY not set. Set MASTER_KEY env var or generate one with: npm run secret:generate',
    );
    process.exit(1);
  }
  return key;
  ...
  try {
    if (options.type === 'file' || !options.type) {
      ...
      `,
        ...
        .all(...(options.days ? [-options.days] : []), options.limit || 100);
...
program.command('show-config').description('Show security configuration').action(cmdShowConfig);
