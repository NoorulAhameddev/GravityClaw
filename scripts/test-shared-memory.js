...
assert(
  fileExists(path.join(SHARED_ROOT, 'projects', 'gravityclaw.json')),
  'projects/gravityclaw.json exists',
);
assert(
  fileExists(path.join(SHARED_ROOT, 'projects', 'aegis-ai.json')),
  'projects/aegis-ai.json exists',
);
assert(fileExists(path.join(SHARED_ROOT, 'logs')), 'logs/ directory exists');
assert(
  fileExists(path.join(VAULT_ROOT, '0-Inbox', 'session-state.md')),
  'vault mirror session-state.md exists',
);
...
assert(
  registry?.sharedOperationalMemory?.startupReads?.length === 3,
  'registry has 3 startupReads',
);
assert(
  registry?.sharedOperationalMemory?.requiredWrites?.length === 3,
  'registry has 3 requiredWrites',
);
assert(
  Array.isArray(registry?.agents) && registry.agents.length === 4,
  'registry registers all 4 agents',
);
assert(
  registry?.writeProtocol?.duringSession?.length >= 2,
  'registry has duringSession write protocol',
);
assert(
  registry?.writeProtocol?.endOfSession?.length >= 2,
  'registry has endOfSession write protocol',
);
...
const handoffs = fs
  .readFileSync(path.join(SHARED_ROOT, 'handoffs.jsonl'), 'utf8')
  .split(/\r?\n/)
  .filter(Boolean);
for (let i = 0; i < handoffs.length; i++) {
  let parsed = null;
  try {
    parsed = JSON.parse(handoffs[i]);
  } catch {}
  assert(parsed !== null, `handoff[${i}] is valid JSON`);
  assert(
    typeof parsed?.agent === 'string' && parsed.agent.length > 0,
    `handoff[${i}].agent is non-empty string`,
  );
  assert(
    typeof parsed?.summary === 'string' && parsed.summary.length > 0,
    `handoff[${i}].summary is non-empty`,
  );
}
console.log('');
...
const decisions = fs
  .readFileSync(path.join(SHARED_ROOT, 'decisions.jsonl'), 'utf8')
  .split(/\r?\n/)
  .filter(Boolean);
for (let i = 0; i < decisions.length; i++) {
  let parsed = null;
  try {
    parsed = JSON.parse(decisions[i]);
  } catch {}
  assert(parsed !== null, `decision[${i}] is valid JSON`);
  assert(
    typeof parsed?.decision === 'string' && parsed.decision.length > 0,
    `decision[${i}].decision is non-empty`,
  );
}
console.log('');
...
assert(
  vaultTimestamp === state?.updatedAt,
  'vault mirror timestamp matches session-state.json updatedAt',
);
console.log('');
